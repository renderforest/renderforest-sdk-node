 ...
 */
const generateLegoScreensPreviews = (projectId, params) => {
...
/**
 * @param {Number} projectId - The project id to cancel preview.
 * @param {Array} queueIds - The queue ids to cancel.
 * @returns {Promise.<Object>}
 */
const cancelLegoPreview = (projectId, queueIds) => {
  const options = {
    method: 'POST',
    endpoint: `${API_PREFIX}/projects/${projectId}/cancel-preview`,
    body: { queueIds }
  }
  return Api.authorizedRequest(options)
}
 ...
 * @param {Number} params.quality - The quality of the preview (e.g., 720).
 * @returns {Promise.<Object>}
 */
const getProjectPreviewUrls = (projectId, params) => {
  return Api.authorizedRequest({
    endpoint: `${API_PREFIX}/projects/${projectId}/preview-urls`,
    qs: params
  })
}
